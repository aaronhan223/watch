...
def retrain_count(conformity_score, training_schedule, sr_threshold, cu_confidence, verbose=False):
...
def training_function(dataset0, dataset0_name, dataset1=None, training_schedule='variable', \
                      ...
                      dataset0_shift_type='none', cov_shift_bias=1.0, plot_errors=False, seed=0, cs_type='signed', \
                        label_uptick=1, verbose=False, noise_mu=0, noise_sigma=0):
    ...
    for score_0 in cs_0:
        m_0, s_0, martingale_value_0, sigma_0 = retrain_count(score_0, training_schedule, sr_threshold, cu_confidence, verbose)
        if m_0:
            retrain_m_count_0 += 1
        if s_0:
            retrain_s_count_0 += 1
    ...
    for score_1 in cs_1:
        m_1, s_1, martingale_value_1, sigma_1 = retrain_count(score_1, training_schedule, sr_threshold, cu_confidence, verbose)
        if m_1:
            retrain_m_count_1 += 1
        if s_1:
            retrain_s_count_1 += 1
