...
def online_lik_ratio_estimates_old(X_cal, X_test_w_est, X_test_0_only, adapt_start=None, classifier='LR'):
...
#     ## Use smaller source set for w estimation
#     X_all_scaled = np.concatenate((X_all_scaled[0:200], X_all_scaled[n_cal:]), axis = 0)
#     adapt_start -= (n_cal - 200)
#     n_cal = 200
#     T = len(X_test_0_only) + n_cal - adapt_start
...
#     print("inside window ahead")
    ...
    ## Scale data
    X_all = np.concatenate((X_cal, X_test_0_only, X_test_w_est), axis=0)
    scaler = preprocessing.StandardScaler().fit(X_all)
    X_all_scaled = scaler.transform(X_all)
    X_test_offline_online = X_all_scaled[adapt_start:]
#     X_cal_test_scaled = np.concatenate((X_all_scaled[0:n_cal], X_all_scaled[n_cal:(n_cal + n_test)]), axis=0)
    X_cal_test_scaled = X_all_scaled[:(n_cal + n_test)]
   
    class_labels_all = np.concatenate((np.zeros(adapt_start), np.ones(T + init_phase)), axis=0)
...
#     idx_include_classifier = np.concatenate(([True], np.repeat(False, adapt_start + init_phase-2), [True], np.repeat(False, T)), axis=0)
    print("adapt_start - len_w_ahead - init_phase : ", adapt_start - len_w_ahead - init_phase)
    print("T - len_w_ahead : ", T - len_w_ahead)
    if (adapt_start < len(X_cal_test_scaled) - 1):
        idx_include_classifier = np.concatenate((np.repeat(True, init_phase + len_w_ahead), np.repeat(False, adapt_start - len_w_ahead - init_phase), np.repeat(True, len_w_ahead), np.repeat(False, T - len_w_ahead), np.repeat(True, init_phase)), axis=0)
    else:
        idx_include_classifier = np.concatenate((np.repeat(True, init_phase), np.repeat(False, adapt_start - init_phase), np.repeat(False, T), np.repeat(True, init_phase)), axis=0)
...
#             print("lik_ratio_model.score() : ", lik_ratio_model.score(X_all_scaled, class_labels_all))
            est_probs = lik_ratio_model.predict_proba(X_cal_test_scaled)
#             print("est_probs : ", est_probs)
#             if (t % 25==0):
#                 print(f'est_probs[{t}] {np.array(est_probs[-5:,1])}')
            if (adapt_start + len_w_ahead + t < len(idx_include_classifier) - init_phase):
                idx_include_classifier[adapt_start + len_w_ahead + t] = True
#                 print("class_labels_all[adapt_start + init_phase + len_w_ahead + t] : ", class_labels_all[adapt_start + init_phase + len_w_ahead + t])
#             if (t < n_cal):
            if (init_phase + len_w_ahead + t < n_cal):
                idx_include_classifier[init_phase + len_w_ahead + t] = True ## Add calibration point
#                 print("class_labels_all[len_w_ahead + t] : ", class_labels_all[len_w_ahead + t])
            
#             print(" np.sum(class_labels_all[idx_include_classifier]) : ", np.sum(class_labels_all[idx_include_classifier]))
#             print("len(class_labels_all[idx_include_classifier]) : ", len(class_labels_all[idx_include_classifier]))
#             print("class_labels_all[:adapt_start]", class_labels_all[:adapt_start])
...
#             print("est_probs_source[] : ", np.mean(est_probs[:,1][:adapt_start]))
#             print("est_probs_test[] : ", np.mean(est_probs[:,1][adapt_start:]))
...
#     idx_include_classifier = np.concatenate((np.repeat(True, adapt_start + init_phase), np.repeat(False, T)), axis=0)
#     idx_include_classifier = np.concatenate(([True], np.repeat(False, adapt_start + init_phase-2), [True], np.repeat(False, T)), axis=0)
    idx_include_classifier = np.concatenate((np.repeat(True, init_phase), np.repeat(False, adapt_start - init_phase), np.repeat(True, init_phase), np.repeat(False, T)), axis=0)
    idx_include_kde_test = np.concatenate((np.repeat(True, init_phase), np.repeat(False, T)), axis=0)  ## init_phase : num initial offline test data available; T : num test points observed online
...
def calculate_weighted_p_values_and_quantiles(args, conformity_scores, W_i, n_cal, x_ctm_alarm, adapt_start, method, depth=1, cs_resampled_cal_list=None, num_test_unshifted=0):
    """
...
#     else:
    ...
    ## Prior to X-CTM raising an alarm, p-values are calculated as in standard CTMs
    wp_values[0:x_ctm_alarm], wq_lower[0:x_ctm_alarm], wq_upper[0:x_ctm_alarm] = \
                                        calculate_p_values_and_quantiles(conformity_scores[0:x_ctm_alarm], args.alpha, args.cs_type)
...
#             print("t_ : ", t_)
#             print("depth : ", depth)
        if (t_ > depth - 1 and method in ['fixed_cal', 'fixed_cal_oracle', 'sliding_window', 'fixed_cal_offline', 'fixed_cal_dyn', 'resample_cal_oracle']):
...
#             print("conformity_scores_t : ", conformity_scores_t[0:10])
        ...
        elif (method in ['one_step_est', 'batch_oracle', 'cyclic_oracle']):
...
#                     print(f'W_i_t : {t_}, {W_i[t_]}')
        ...
        ## Normalize weights on subset of weights
#         print(f'normalized_weights_{t_} : {W_i_t}')
        normalized_weights_t = W_i_t / np.sum(W_i_t)
#         print(f'normalized_weights_{t_} : {normalized_weights_t}')
#         print(f'{method} eff sample size : {effective_sample_size_est(normalized_weights_t)}')
        ...
        if method in ['one_step_oracle', 'one_step_est', 'batch_oracle', 'multistep_oracle', 'cyclic_oracle']:
            idx_include_inf[adapt_start+t_] = True


        if (np.sum(normalized_weights_t[conformity_scores_t == conformity_scores_t[-1]]) < args.alpha):
#             if (np.sum(normalized_weights_t[n_cal:]) < args.alpha):
            ...
            print("Using anticonservative p-value, normalized_weights_t[-1] : ", normalized_weights_t[-1])

                
                

#     elif (method in ['one_step_oracle', 'one_step_est', 'batch_oracle', 'multistep_oracle', 'cyclic_oracle']):
...
#             if (method in ['one_step_est', 'batch_oracle', 'cyclic_oracle']):
#                 W_i_t = W_i[t_][:(adapt_start+t_+1)]
...
#             ## Normalize weights on subset of weights
#             normalized_weights_t = W_i_t / np.sum(W_i_t)
# #             print("normalized_weights_t : ", normalized_weights_t)
